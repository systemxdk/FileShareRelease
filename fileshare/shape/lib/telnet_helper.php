 ...
 * shape Telnet class
 * 
 * All failueres will result in an exception
 */
class shTelnet {
	
	/**
	 * contruct specvific attributes
	 */
	private $host;
	private $port;
	private $timeout;
	...
	/**
	 * Telnet specific attributes.... see contructor
	 *
	 * @var unknown_type
	 */
	private $NULL;
	...
	/**
	 * true/false constants for return values
	 *
	 */
	 ...
	 * @return void
	 */
	public function __construct($host = '127.0.0.1', $port = '23', $timeout = 10, $newline = "\r\n"){
	 ...
	 * @param string $stdpromt Standard promt (defaults to $)
	 * @param string $loginpromt login promt (defaults to 'login:')
	 * @param string $loginpromt login promt (defaults to 'Password:')
	 * @return boolean 
	 */
	public function login($username, $password, $stdpromt = '$', $loginpromt = 'login:', $passwordpromt = 'Password:') {
		
		/**
		 * notice that we reset the prompt before writing, this is to avoid confusion, and not neccesary
		 */
		try{
			$this->setPrompt($loginpromt);
			$this->waitPrompt();
			$this->setPrompt($passwordpromt);
			$this->write($username);
			$this->waitPrompt();
			$this->setPrompt($stdpromt);
			$this->write($password);
			$this->waitPrompt();
	...
	private function getBuffer(){
		...
		$buf = explode($this->newline, $this->buffer);
		unset($buf[count($buf)-1]);
		$buf = implode($this->newline,$buf);
		return trim($buf);
	}
